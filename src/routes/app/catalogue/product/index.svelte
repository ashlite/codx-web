...
<div class="flex justify-between items-center gap-4">
  ...
  <div class="w-60">
    <button class="btn btn-primary w-full" on:click={() => globalModal.editProduct({})}>Add New Product</button>
  </div>
</div>
      ...
      {#if listProduct.data.length > 0}
        {#each listProduct.data as product}
          <tr>
            ...
            <td>
              <div class="btn-group">
                <button class="btn btn-sm btn-warning" on:click={() => globalModal.editProduct(product, product.collection)}>
                ...
                <button class="btn btn-sm btn-error" on:click={() => globalModal.deleteConfirmation(product.id, product.name, 'product')}>

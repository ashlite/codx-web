<script>
  ...
  async function handleDelete(id){
    try{
      const response = await del(`/customer/${id}`)
      if (response.status == 200){
        toastAlert.success('Customer deleted')
      } else {
        toastAlert.success('Customer failed to delete')
      }
      ...
      {#if listCustomer.data.length > 0}
        {#each listCustomer.data as customer}
          <tr>
            ...
            <td>
              <div class="btn-group">
                <button class="btn btn-sm btn-warning" on:click={() => globalModal.editCustomer(customer)}>
                ...
                <button class="btn btn-sm btn-error" on:click={() => handleDelete(customer.id)}>
